...
switch ($action) {
  case 'list': // filtros: codigo_finca, codigo_rol, activo
    requireLogin();
    $f = [
      'codigo_finca' => $_GET['codigo_finca'] ?? null,
      'codigo_rol'   => $_GET['codigo_rol'] ?? null,
  ...
  case 'create':
    ...
    $ok = $model->crear($d);
    $ok ? out(['ok'=>true]) : out(['error'=>'No se pudo crear (valida password y email únicos)'],400);
  case 'update':
    requireLogin();
